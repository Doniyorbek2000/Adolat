...
@Injectable()
export class GovUzConnector extends BaseSourceConnector {
  readonly sourceName = 'gov.uz';
  private readonly logger = new Logger(GovUzConnector.name);
  private readonly baseUrl = 'https://www.gov.uz';

  async fetchDocuments(limit = 10): Promise<FetchedDocument[]> {
    try {
      const response = await this.http.get<string>(`${this.baseUrl}/uz/open_data/`);
      ...
      $('a[href]').each((_, el) => {
        ...
        if (href.startsWith('/') && title.length > 5) {
