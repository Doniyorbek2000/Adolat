...
@Injectable()
export class LexUzConnector extends BaseSourceConnector {
  readonly sourceName = 'lex.uz';
  private readonly logger = new Logger(LexUzConnector.name);
  private readonly baseUrl = 'https://lex.uz';

  async fetchDocuments(limit = 10): Promise<FetchedDocument[]> {
    try {
      const response = await this.http.get<string>(`${this.baseUrl}/docs`);
      const $ = cheerio.load(response.data);
      const links: Array<{ href: string; title: string }> = [];

      // Extract document links — lex.uz lists legal documents in <a> tags
      $('a[href]').each((_, el) => {
        const href = $(el).attr('href') ?? '';
        const title = $(el).text().trim();
        if (href.includes('/docs/') && title.length > 5) {
      ...
      for (const link of uniqueLinks) {
        try {
          ...
          this.logger.warn(`Failed to fetch document ${link.href}: ${String(err)}`);
        }
      }
